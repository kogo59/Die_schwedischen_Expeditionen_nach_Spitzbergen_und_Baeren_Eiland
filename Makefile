BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Die_schwedischen_Expeditionen_nach_Spitzbergen_und_Baeren_Eiland
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Die_schwedischen_Expeditionen_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Die_schwedischen_Expeditionen_*.jpg 
